			...
			case 'stage': //BOFA
				{
						...
						stageCurtains.scrollFactor.set(1.4, 1.4);
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				for (susNote in 0...Math.floor(susLength))
				{
					...
					{
						sustainNote.x += FlxG.width / 2 + 9; // general offset
					}
				}
			...
			switch (noteTypeCheck)
			{
				case 'pixel':
					...
					case 'normal':
						...
						switch (Math.abs(i))
						{
							case 0:
								...
								babyArrow.animation.addByPrefix('confirm', 'left press', 24, false);
							case 1:
								babyArrow.x += Note.swagWidth * 1;
								babyArrow.animation.addByPrefix('static', 'arrowDOWN');
								babyArrow.animation.addByPrefix('pressed', 'down press', 24, false);
								babyArrow.animation.addByPrefix('confirm', 'down press', 24, false);
							case 2:
								babyArrow.x += Note.swagWidth * 2;
								babyArrow.animation.addByPrefix('static', 'arrowUP');
								babyArrow.animation.addByPrefix('pressed', 'up press', 24, false);
								babyArrow.animation.addByPrefix('confirm', 'up press', 24, false);
							case 3:
								babyArrow.x += Note.swagWidth * 3;
								babyArrow.animation.addByPrefix('static', 'arrowRIGHT');
								babyArrow.animation.addByPrefix('pressed', 'right press', 24, false);
								babyArrow.animation.addByPrefix('confirm', 'right press', 24, false);
							}
					...
					default:
						...
						switch (Math.abs(i)) //come back here
						{
							case 0:
								babyArrow.x += Note.swagWidth * 0;
								babyArrow.animation.addByPrefix('static', 'arrowLEFT');
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null)
		{
			...
			if (PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection && camFollow.x != boyfriend.getMidpoint().x - 100)
			{
				...
				switch (curStage)
				{
					...
					// BOFA
					case 'stage':
						camFollow.x = boyfriend.getMidpoint().x - 1025;
	...
	private function popUpScore(daNote:Note):Void
		{
			...
			rating.screenCenter();
			rating.y = coolText.y + 750;
			rating.x = coolText.x - 750;
		...
		private function keyShit():Void // I've invested in emma stocks
			{
				...
				playerStrums.forEach(function(spr:FlxSprite)
				{
					...
					if (spr.animation.curAnim.name == 'confirm' && !curStage.startsWith('stage'))
					{
						spr.centerOffsets();
						spr.offset.x -= 13;
						spr.offset.y -= 13;
